...
TEST(JobControlRootTest, StealTTY) {
  ...
  if (!child) {
    ASSERT_THAT(setsid(), SyscallSucceeds());
    if (!ioctl(slave.get(), TIOCSCTTY, 0)) {
      // We were able to steal the terminal with the wrong arg value.
      _exit(1);
    }
    if (ioctl(slave.get(), TIOCSCTTY, 1)) {
      // We weren't able to steal the terminal with the correct arg value.
      _exit(2);
    }
    _exit(0);
  }
