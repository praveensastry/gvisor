...
func newBlockMapFile(regFile regularFile) (*blockMapFile, error) {
	...
	blksUsed := regFile.blksUsed(regFile.inode.blkSize)
	if blksUsed < toFill {
		toFill = blksUsed
	}
...
func (f *blockMapFile) read(curPhyBlk uint32, relFileOff uint64, height uint, dst []byte) (int, error) {
	...
	if height == 0 {
		canRead := int(f.regFile.inode.blkSize - relFileOff)
		toRead := canRead
		if len(dst) < canRead {
			toRead = len(dst)
		}
	...
	for i := startIdx; i < endIdx; i++ {
		...
		err := readFromDisk(f.regFile.inode.dev, curPhyBlkOff+int64(i<<2), &childPhyBlk)
		if err != nil {
			return read, err
		}
