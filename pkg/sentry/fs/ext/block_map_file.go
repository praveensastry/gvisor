...
const (
	// numDirectBlks is the number of direct blocks in ext block map indoes.
	numDirectBlks = 12
)
...
type blockMapFile struct {
	...
	// these holds filedata. Contains file blocks 0 to 11.
...
// extentReader implements io.Reader which will fetch fill data from the block
// maps and build the blockMapFile.fileToPhyBlks array if required.
...
// Compiles only if extentReader implements io.Reader.
...
func (r *blockMapReader) read(curPhyBlk uint32, relFileOff uint64, height uint, dst []byte) (int, error) {
	...
	if height == 0 {
		canRead := int(r.blkSize - relFileOff)
		toRead := canRead
		if len(dst) < canRead {
			toRead = len(dst)
		}
	...
	for i := startIdx; i < endIdx; i++ {
		...
		err := readFromDisk(r.dev, curPhyBlkOff+int64(i<<2), &childPhyBlk)
		if err != nil {
			return read, err
		}
