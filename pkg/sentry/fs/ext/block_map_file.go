...
type blockMapFile struct {
	...
	// index. This is initialized (at max) with the first 12 entries. Rest have to
	// be read in from disk when required.
...
func (f *blockMapFile) Read(ctx context.Context, dev io.ReadSeeker, blkSize uint64, dst usermem.IOSequence, offset int64) (int64, error) {
	panic("unimplemented")
}
...
func newBlockMapFile(dev io.ReadSeeker, blkSize uint64, regFile regularFile) (*blockMapFile, error) {
	file := &blockMapFile{regFile: regFile}
	file.regFile.impl = file

	toFill := uint64(12)
	if regFile.blksUsed < toFill {
		toFill = regFile.blksUsed
	}
