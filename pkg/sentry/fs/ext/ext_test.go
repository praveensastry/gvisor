...
// setUp opens the filesystem image passed as a ext Filesystem and returns all
// necessary elements required to run tests. If error is non-nil, it also
// returns a tear down function which must be called after the test is run
// for clean up.
...
func TestFilesystemInit(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Offload superblock and block group descriptors contents into local structs
			// for comparison.
