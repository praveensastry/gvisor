...
type ThreadGroup struct {
	...
	// isTTYMaster is whether tty points to a terminal master. If false, it
	// points to a slave.
	isTTYMaster bool
}
...
//
// Precondition: tty.Mu must be held.
func (tg *ThreadGroup) SetControllingTTY(tty *TTY, arg int32, isMaster bool) error {
...
//
// Precondition: tty.Mu must be held.
func (tg *ThreadGroup) ReleaseControllingTTY(tty *TTY, isMaster bool) error {
	// We might be asked to set the controlling terminal of multiple
	// processes, so we lock both the TaskSet and SignalHandlers.
	...
	if tg.tty == nil || tg.tty != tty || tg.isTTYMaster != isMaster {
		tg.signalHandlers.mu.Unlock()
...
func (tg *ThreadGroup) SetForegroundProcessGroup(tty *TTY, pgid ProcessGroupID) (int32, error) {
	...
	// TODO(b/b/129283598): "If tcsetpgrp() is called by a member of a
